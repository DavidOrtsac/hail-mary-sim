#!/bin/bash
# MARSSHINE-1 — one-command GCP provision. Runs on your laptop.
...
VM_NAME="${VM_NAME:-marsshine-stream}"
...
if [ -z "$AMBIENT_GCS_URL" ]; then
  read -rp "  GCS path to ambient.mp3 (e.g. gs://marsshine-ops/ambient.mp3): " AMBIENT_GCS_URL
fi
...
POLICY_NAME="MARSSHINE stream broken — CPU dropout"
...
${BOLD}Watch first-boot progress:${RESET}
  gcloud compute ssh $VM_NAME --zone $ZONE --project $PROJECT \\
    -- "sudo tail -f /var/log/marsshine-startup.log"

${BOLD}Once running:${RESET}
  # Stream service status
  gcloud compute ssh $VM_NAME --zone $ZONE -- "sudo systemctl status marsshine-stream"

  # Live tail of chromium + ffmpeg
  gcloud compute ssh $VM_NAME --zone $ZONE -- "sudo journalctl -u marsshine-stream -f"

  # Force chromium reload (manual deploy refresh)
  gcloud compute ssh $VM_NAME --zone $ZONE -- "sudo systemctl restart marsshine-stream"
